import asyncio, re, toml, aiohttp, colorama, itertools, traceback
...
try:
    config = toml.load(open("dcsc_config.toml", "r", encoding="utf-8"))
except:
    config = {
    ...
    toml.dump(config, open("dcsc_config.toml", "w", encoding="utf-8"))
...
async def change_status(session:aiohttp.ClientSession, status_inf:dict={"status": str, "message": str}, emoji:dict={"name":str, "id":str, "animated":bool}):
    ...
    try:
        resp = await session.patch(url=config["api_url"], headers=header, json=json)
        color = (colorama.Fore.GREEN) if resp.status == 200 else (colorama.Fore.YELLOW)
        print(color + f"[{resp.status} {resp.reason}]")
        if resp.status != 200:
...
async def main():
    print(colorama.Fore.BLUE + "status changer running, press ctr+c to exit")
    session = aiohttp.ClientSession()
    statuses = itertools.cycle(config["statuses"])
    while True:
        status, emoji_name, emoji_id, emoji_animated, message = process_status(next(statuses))
        if status:
...
try:
    ...
    print(colorama.Fore.YELLOW + f"warning: {get_traceback()}")
except Exception as e:
    print(colorama.Fore.RED + f"exception: {get_traceback()}")
except BaseException as b:
    print(colorama.Fore.RED + colorama.Style.BRIGHT + f"error: {get_traceback()}")